...
contract Farm is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function resetAndsetStartEndBlock(uint256 _rewardAmount, uint256 _start, uint256 _end) external onlyOwner returns (bool) {
        require(
            _start > currentBlock(),
            "Start should be more than current block"
        );
    ...
    */
    ...
    function stake(uint256 amount) external _realAddress(msg.sender) _hasAllowance(msg.sender, amount, tokenAddress) returns (bool) {
    ...
    function emergencyWithdraw()external _realAddress(msg.sender) returns (bool){
    ...
    function _withdraw(address from) private nonReentrant() returns (bool) {
        updateShare();
        uint256 amount = deposits[from].amount;
